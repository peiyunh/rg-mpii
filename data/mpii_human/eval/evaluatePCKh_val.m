function evaluatePCKh_val(predidxs,bSave)
...
%load([p.gtDir '/mpii_human_pose_v1_u12_1'],'RELEASE');
load([p.gtDir '/mpii_human_pose_remarked.mat']);
for i = 1:numel(anno_rect)
    annolist(i).annorect = anno_rect{i};
end
annolist_val_flat = annolist(image_set==2);
single_person_val_flat = single_person(image_set==2);

% evaluate on the "single person" subset only
%single_person_test = RELEASE.single_person(RELEASE.img_train == 1);
...
gt = annolist2matrix(annolist_val_flat(single_person_val_flat == 1));
% compute head size
headSize = getHeadSizeAll(annolist_val_flat(single_person_val_flat == 1));

pckAll = zeros(length(range),16,length(predidxs));

%model_names = {'qp1-fcn8s', ...
%               'qp2-fcn8s', ...
%               'qp1-fcn4s-ep2', ...
%               'qp1-fcn4s-ep3', ...
%               'qp1-fcn4s-ep4', ...
%               'qp2-fcn4s-ep1', ...
%              };
%model_paths = {'models/bottomup-multitask-mpii-fcn-8s/result-epoch-10.mat',...
%               'models/topdown-multitask-mpii-fcn-8s/result-epoch-10.mat', ...
%               'models/bottomup-multitask-mpii-fcn-4s/result-epoch-2.mat', ...
%               'models/bottomup-multitask-mpii-fcn-4s/result-epoch-3.mat',...
%               'models/bottomup-multitask-mpii-fcn-4s/result-epoch-4.mat',...
%               'models/topdown-multitask-mpii-fcn-4s/result-epoch-1.mat', ...
%               'models/topdown-multitask-mpii-fcn-4s/result-epoch-4.mat', ...
%              };

%predidxs = 1:numel(model_names);
for i = 1:length(predidxs);
    % load predictions
    p = getExpParams(predidxs(i));
    load(p.predFilename, 'pred');
    
    %assert(length(annolist_val) == length(pred));
    %pred_flat = flatten_annolist(pred,single_person_val);
    pred_flat = pred;
    pred = annolist2matrix(pred_flat(single_person_val_flat == 1));
